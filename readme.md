...
Once the binary is loaded into memory through any method, you can run it with `call 49152`. Note that the binary size is currently unoptimized, and appears to already exceed the size that a 24k machine can save for later via the `savem` command.
